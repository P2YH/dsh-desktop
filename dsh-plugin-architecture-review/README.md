...
From **资料与知识**, **维护知识** starts a DSH session rooted in the selected workspace. Enter a question and choose **查询** to check the wiki against original sources. Progress and answers remain in the workbench, where you can stop a running task or ask follow-up questions in the same session. **资料核对** from a review detail opens its result in the same workbench view. These agent tasks are separate from the Host's deterministic material-import feedback; findings and decisions remain subject to human confirmation. The skill is also available directly as `/architecture-review-knowledge` in a DSH session.

The review workbench has four stages: **资料准备 → 专家评审 → 人工确认 → 决策**. Creating a project saves a draft and its optional 待评审资料. Each submission immediately reports which files have readable text and which stored originals need an OCR, Markdown, or TXT supplement; expert review requires at least one readable source. Choose the participating experts and start the review directly. Collaboration through the Profile's public `subagent` tool is always enabled. At startup, the Host derives the applicable Markdown standards under `raw/sources/standards/` from the selected experts' catalog references and reports any missing originals together before dispatching experts. The Host records hashes of the source and standards snapshot and prevents duplicate runs. Tool results are matched to expert IDs; a call without a result never counts as completion. Run ID, session ID, individual status and original conclusions persist in `expert-run.json`. The workbench restores the session on return and can retry a failed expert alone.
